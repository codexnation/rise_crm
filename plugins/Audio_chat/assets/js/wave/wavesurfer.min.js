...
function n(t, e) {
    ...
    for (const [t, s] of Object.entries(e))
        if ("children" === t)
            for (const [t, s] of Object.entries(e)) "string" == typeof s ? i.appendChild(document.createTextNode(s)) : i.appendChild(n(t, s));
            ...
            return function(e, i) {
                t(this, void 0, void 0, (function*() {
                    ...
                    const n = e.body.getReader(),
                        s = Number(e.headers.get("Content-Length")) || 0;
                    let r = 0;
                    const o = e => t(this, void 0, void 0, (function*() {
                            r += (null == e ? void 0 : e.length) || 0;
                            const t = Math.round(r / s * 100);
                            i(t)
                        })),
...
class h extends e {
    constructor(t, e) {
        ...
        $(".change-theme").click(function () {
            var resp = _this.initHtml();
            console.log(resp);
        /*var color = getCookie("theme_color") || AppHelper.settings.defaultThemeColor;
        if (color && color == "1E202D") {
            $('#waveform').append('<div id="dark_player_css"><style> :host audio::-webkit-media-controls-panel { background-color: #1c1f26; } :host audio :host audio::-webkit-media-controls-play-button { background-color: #2f3541; border-radius: 50%; } :host audio :host audio::-webkit-media-controls-time-remaining-display { color: #5b73e8; text-shadow: none; } :host audio :host audio::-webkit-media-controls-current-time-display { color: #5b73e8; text-shadow: none; } :host audio :host audio::-webkit-media-controls-timeline { background-color: #2f3541; border-radius: 25px; margin-left: 10px; margin-right: 10px; } :host audio :host audio::-webkit-media-controls-volume-slider { background-color: #2f3541; border-radius: 25px; padding-left: 8px; padding-right: 8px; } :host audio :host audio::-webkit-media-controls-mute-button { background-color: #2f3541; border-radius: 50%; }</style></div>');
        }else{
            $('#dark_player_css').remove();
        }*/
        });
    ...
    initEvents() {
        ...
        this.wrapper.addEventListener("click", (e => {
            ...
            const {
                ...
                scrollWidth: e,
                clientWidth: i
            } = this.scrollContainer, n = t / e, s = (t + i) / e;
    ...
    initDrag() {
        this.subscriptions.push(function(t, e, i, n, s = 3, r = 0, o = 100) {
            if (!t) return () => {};
            ...
            const l = l => {
                ...
                const p = Date.now(),
                    m = n => {
                        if (n.preventDefault(), n.stopPropagation(), a && Date.now() - p < o) return;
                        const r = n.clientX,
                            h = n.clientY,
                            l = r - d,
                            m = h - c;
                        if (u || Math.abs(l) > s || Math.abs(m) > s) {
                            const n = t.getBoundingClientRect(),
                                {
                                    left: s,
                                    top: o
                                } = n;
                            u || (null == i || i(d - s, c - o), u = !0), e(l, m, r - s, h - o), d = r, c = h
                        }
                    },
                    f = e => {
                        if (u) {
    ...
    initHtml(theme_updated = false) {
        ...
        var dark_css = '';
        if (color && color == "1E202D") {
            var href = AppHelper.assetsDirectory + "../plugins/Audio_chat/assets/css/audio_chat_dark.css";
            $('#project-audio_chat-section').append('<link id="audio_chat-theme-color" class="audio_chat-theme-color" rel="stylesheet" href="' + href + '" type="text/css" />');
            dark_css = `<style id="dark_css">
                        :host audio {
                          border-radius: 50px;
                        ...
                        :host audio::-webkit-media-controls-background {
                          background-color: #1c1f26;
                          display:none;
                        }
                        :host audio::-webkit-media-controls-panel {
                          background-color: #1c1f26;
                          border: none;
                        }
                        :host audio::-webkit-media-controls-play-button,
                        :host audio::-webkit-media-controls-pause-button,
                        :host audio::-webkit-media-controls-seek-back-button,
                        :host audio::-webkit-media-controls-seek-forward-button,
                        :host audio::-webkit-media-controls-mute-button,
                        :host audio::-webkit-media-controls-fullscreen-button,
                        :host audio::-webkit-media-controls-rewind-button,
                        :host audio::-webkit-media-controls-return-to-realtime-button,
                        :host audio::-webkit-media-controls-toggle-closed-captions-button,
                        :host audio::-webkit-media-controls-menu-button {
                        ...
                        :host audio::-webkit-media-controls-volume-slider-container,
                        :host audio::-webkit-media-controls-volume-slider{
                            border-radius:0;
                            color:#fff;
                        }

                        :host audio::-webkit-media-controls-menu-button,
                        :host audio::-webkit-media-controls-overflow-button {
                          display: none;
                        }
                        :host audio::-webkit-media-controls-volume-slider-container,
                        :host audio::-webkit-media-controls-volume-slider,
                        ...
                        :host audio::-webkit-media-controls-current-time-display,
                        :host audio::-webkit-media-controls-time-remaining-display {
                          color: #FFFFFF;
                          text-shadow: none;
                        }

                        :host audio::-webkit-media-controls-timeline {
                          color: #ffffff; /* For the slider thumb */
                        }

                        :host audio::-webkit-media-controls-timeline-container {
                          color: #ffffff; /* For the timeline background */
                        }

                        :host audio::-webkit-media-controls-timeline-bar {
                          background-color: #ffffff; /* For the played part of the timeline */
                        }
                        #waveform #wrapper .timeline-wrapper {
                            color : #fff !important;
                        }
                        </style>`;
            $('.project-details-view').addClass('dark-audio-enabled');
            if( $('.project-details-view').index() != -1 && $('.project-details-view').hasClass('light-audio-enabled')){
                window.location.reload();
            }
        }
        else{
            $(".audio_chat-theme-color").remove();
            /*setTimeout(function(){
                console.log(e.querySelector('style'));
            },3000);*/
            $('.project-details-view').addClass('light-audio-enabled');
            if( $('.project-details-view').index() != -1 && $('.project-details-view').hasClass('dark-audio-enabled')){
                window.location.reload();
            }
            
        }
        return e.innerHTML = dark_css + `\n<style> :host {
                                                user-select: none;
                                                min-width: 1px;
                                            }
    ...
    renderBarWaveform(t, e, i, n) {
        ...
        for (let t = 0; t <= o; t++) {
            ...
            if (o > g) {
                const t = Math.round(v * l * n),
                    s = t + Math.round(b * l * n) || 1;
                let r = l - t;
                "top" === e.barAlign ? r = 0 : "bottom" === e.barAlign && (r = h - s), i[f](g * (c + u), r, c, s, p), g = o, v = 0, b = 0
            }
    ...
    renderSingleCanvas(t, e, i, n, s, r, o, a) {
        const h = window.devicePixelRatio || 1,
            l = document.createElement("canvas"),
            d = t[0].length;
        l.width = Math.round(i * (r - s) / d), l.height = n * h, l.style.width = `${Math.floor(l.width/h)}px`, l.style.height = `${n}px`, l.style.left = `${Math.floor(s*i/h/d)}px`, o.appendChild(l);
        const c = l.getContext("2d");
        if (this.renderWaveform(t.map((t => t.slice(s, r))), e, c), l.width > 0 && l.height > 0) {
            const t = l.cloneNode(),
                i = t.getContext("2d");
    ...
    renderChannel(e, i, n, s) {
        ...
        return t(this, void 0, void 0, (function*() {
            ...
            i.style.height = `${a}px`, r && s > 0 && (i.style.marginTop = `-${a}px`), this.canvasWrapper.style.minHeight = `${a}px`, this.canvasWrapper.appendChild(i);
            const l = i.cloneNode();
            this.progressWrapper.appendChild(l);
            const d = e[0].length,
                c = (t, s) => {
                    this.renderSingleCanvas(e, o, n, a, Math.max(0, t), Math.min(s, d), i, l)
                };
            if (!this.isScrollable) return void c(0, d);
            const {
                scrollLeft: u,
                scrollWidth: p,
                clientWidth: m
            } = this.scrollContainer, f = d / p;
            let g = Math.min(h.MAX_CANVAS_WIDTH, m);
            if (o.barWidth || o.barGap) {
                const t = o.barWidth || .5,
                    e = t + (o.barGap || t / 2);
                g % e != 0 && (g = Math.floor(g / e) * e)
            }
            const v = Math.floor(Math.abs(u) * f),
                b = Math.floor(v + g * f),
                y = b - v;
            y <= 0 || (c(v, b), yield Promise.all([(() => t(this, void 0, void 0, (function*() {
                if (0 === v) return;
        ...
        return t(this, void 0, void 0, (function*() {
            ...
            try {
                if (this.options.splitChannels) yield Promise.all(Array.from({
                    ...
                    var n;
                    const s = Object.assign(Object.assign({}, this.options), null === (n = this.options.splitChannels) || void 0 === n ? void 0 : n[i]);
                    return this.renderChannel([e.getChannelData(i)], s, r, i)
                })));
                else {
    ...
    scrollIntoView(t, e = !1) {
        const {
            ...
            scrollWidth: n,
            clientWidth: s
        } = this.scrollContainer, r = t * n, o = i, a = i + s, h = s / 2;
        if (this.isDragging) {
            const t = 30;
...
class d extends e {
    ...
    stopAt(t) {
        ...
        null === (e = this.bufferNode) || void 0 === e || e.stop(this.audioContext.currentTime + n), null === (i = this.bufferNode) || void 0 === i || i.addEventListener("ended", (() => {
...
class u extends a {
    ...
    loadAudio(e, n, s, r) {
        var a;
        return t(this, void 0, void 0, (function*() {
            if (this.emit("load", e), !this.options.media && this.isPlaying() && this.pause(), this.decodedData = null, !n && !s) {
            ...
            if (s) this.decodedData = i.createBuffer(s, t || 0);
            else if (n) {
